...
A repo is not considered stable merely because files were edited. It is stable when the intended production path is clear, conflicting old files are removed or documented, the site or app has a defined browser QA path, deployment settings are known or documented, and future agents know what not to undo.
